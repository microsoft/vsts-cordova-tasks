...
6.  As an optional step, you can configure your build to upload the resulting build artifacts to your TFS or VSTS instance for easy access. The sample gulpfile.js script places the resulting output in the "bin" folder to make configuration simple.
...
If you are not using Visual Studio Team Services's (VSTS) Hosted Agent Pool or MacinCloud's streamlined [VSTS plan](http://go.microsoft.com/fwlink/?LinkID=691834), you can also use your own hardware. Because of its design, you can also easily install the cross-platform agent an on-premise Windows or Mac and integrate with either TFS or Visual Studio Team Services. The build machine simply needs to have HTTP access to to the server with your TFS collection or Visual Studio Team Services. 
