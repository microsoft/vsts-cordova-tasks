...
Visual Studio Online (VSO) and Team Foundation Services (TFS) 2015 can both be used for building and testing Cordova apps in a Continous Integration (CI) environment thanks to a new [cross-platform agent](http://go.microsoft.com/fwlink/?LinkID=533789) that enables VSO or TFS to build directly on Windows or OSX. Further, [Tools for Apache Cordova](https://www.visualstudio.com/en-us/features/cordova-vs.aspx) is designed to work with a number of different CI systems since the projects it creates are standard [Apache Cordova Command Line interface](http://go.microsoft.com/fwlink/?LinkID=533773) (CLI) projects. The end result is you can use VSO or TFS to build projects created using Tools for Apache Cordova or any Cordova compliant CLI project like Ionic or the TACO CLI.
...
2.  Now we will add a Cordova Build task for Android 
	...
	2.  Use the following settings:
		...
		- **Android**: Set these values if you want to sign a release build. See [securiting your signing keys](./secure-certs.md) for details.
...
Using Gulp in a CI environment is fairly straight forward as you can see in the detailed [Gulp tutorial](../tutorial-gulp/gulp-ci.md). Cordova itself does not have built in TypeScript support, but Gulp can be configured easily to compile TypeScript, LESS, SASS, or anything else you may want. You can even use it to run your tests!

####TypeScript Example
To add TypeScript compilation into your build defintion using Gulp, follow these steps:
...
>**Troubleshooting Tip**: *Be sure to use a version of Node.js < 4.0.0 on OSX if you are using Cordova < 5.3.3.* One of Cordova's iOS related dependencies has comaptibility issues with Node.js 4.0.0 which were resolved in 5.3.3.
