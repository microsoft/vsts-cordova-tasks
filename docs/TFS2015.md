...
# Build Apache Cordova apps with Visual Studio Team Services or Team Foundation Services 2015
...
**Tutorial sections:**
- [Installing the VSTS Extension](#install)
...
<a name="vso"></a>
### Visual Studio Team Services Hosted Agents
As of this writing, you can build Cordova apps targeting Android, Windows, and Windows Phone using the Hosted Agent Pool in Visual Studio Team Services. This allows you to build without setting up a Windows build agent on premise. For iOS builds you can install the  [VSTS cross-platform build agent](http://go.microsoft.com/fwlink/?LinkID=533789) on an OSX machine if you intend to build iOS. MacinCloud provides a [special plan](http://go.microsoft.com/fwlink/?LinkID=691834) and streamlined setup experience for VSTS agents in the cloud. All Cordova pre-requisites should already be installed and configured when using the Hosted Agent Pool in Visual Studio Team Services (VSTS) or MacinCloud's special VSTS plan. Cordova versions not already present will be dynamically downloaded during your build.

See [agent setup](#agentsetup) below for details on setting up a build agent if you are either not using Visual Studio Team Services or want to use your own Mac to build for iOS
...
1.  Create a new build definition by selecting the "Build" tab for your VSTS/TFS project, clicking the "+" icon, and selecting "Empty" as the template. We'll start out targeting platforms that can be built on Windows so give the build definition a name that indicates that this is the case.
...
6.  As an optional step, you can configure your build to upload the resulting build artifacts to your TFS or VSTS instance for easy access.
...
3. Update your build definition as follows:
  1. Update the **Platform** value for the **Cordova Build** step to be **$(Platform)** and update any platform specific options as appropriate.
...
2. Next, we will configure the definition to publish your test results to VSTS.
  ...
  2. Use the following settings:
    ...
    - **Advanced &gt; Always Run:** Be sure to check this option so your test results are published when the tests fail.
...
If you are not using the Visual Studio Team Services (VSTS) Hosted Agent Pool or MacinCloud's streamlined [VS Team Services plan](http://go.microsoft.com/fwlink/?LinkID=691834), you can use your own hardware instead. Because of its design, you can easily install the cross-platform agent an on-premise Windows or Mac and integrate with either TFS or Visual Studio Team Services. The build machine simply needs to have HTTP access to the server with your TFS collection or Visual Studio Team Services. 
...
1. Since the build process we will describe here is not directly dependent on MSBuild or Visual Studio for Android, you have two options for installing pre-requisites on Windows:
  ...
  3. See "Installing Dependencies" in the [general Tools for Apache Cordova CI tutorial](http://go.microsoft.com/fwlink/?LinkID=691196) for additional details.

2. Next you will need to install the Windows build agent to build Android, Windows, or Windows Phone, and the [cross-platform build agent](http://go.microsoft.com/fwlink/?LinkID=533789) on a Mac if you intend to build iOS (and Android if you so desire). 
...
Note: All Cordova pre-requisites should already be installed and configured when using the Hosted Agent Pool in Visual Studio Team Services (VSTS) or agents from MacinCloud's special VSTS agent plan. Cordova versions not already present will be dynamically downloaded during your build. You can skip this section if you are only using the VSTS Hosted Agent Pool.
...
- **%ANT_HOME%\\bin** (or $ANT_HOME\bin on OSX/Linux) should be added to your path if you are using a version of Cordova < 5.0.0 or have specified checked the "Force Ant" build option.
