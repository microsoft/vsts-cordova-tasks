...
- **TFS 2015 Update 1 and Earlier**: TFS 2015 Update 1 does not currently support the VSO extensibility model. Fortunatley installation is still relativley easy to do by following these simple steps.
...
We will acomplish this by using the concept of a "demand" in two separate build definitions to route the work to the correct OS.

#### Definition for Building Android on Windows
...
2. Change the "Platform" value to "ios" 
...
You can also use the exact same Gulp build steps above to run your tests! See the [Cordova test tutorials](../tutorial-testing-cordova) for details on setting up Gulp. Just update the Gulp Task(s) to reference your "test" task and drop it in the right place in your build workflow!
...
<table>
    ...
    <tr>
      <td><strong>GRADLE\_USER\_HOME</strong></td>
      <td>Optional</td>
      ...
      <td>If not specified, uses %HOME%\\.gradle on Windows or ~/.gradle on OSX</td>
    </tr>
    <tr>
      <td><strong>CORDOVA\_CACHE</strong></td>
      <td>Optional</td>
      <td>Overrides the default location used by the Cordova Build Task to cache installs of multiple versions of Cordova.</td>
      <td>If not specified, uses %APPDATA%\\cordova-cache on Windows and ~/.cordova-cache on OSX</td>
    </tr>
  </tbody>
</table>
