...
For this reason, this tutorial will focus on the use of the cross-platform agent rather than the XAML/MSBuild based build setup for Cordova apps. If you still need to use the legacy XAML / MSBuild based approach, see the [TFS 2013](http://go.microsoft.com/fwlink/?LinkID=533770) tutorial for details on setup. The instructions generally still apply to TFS 2015.

*Note: The instructions shown here also apply to Apache Cordova CLI apps built outside of Visual Studio along with CLI compatible frameworks and tools like Ionic.*

>**Troubleshooting Tip**: Be aware that we recommend against adding the "platforms" folder or the following json files in the "plugins" folder into source control: android.json, ios.json, remote_ios.json, windows.json, and wp8.json. See "What to Add to Source Control" in the [general CI guide](http://go.microsoft.com/fwlink/?LinkID=691193) for additional details.
...
See [agent setup](#agentsetup) for details on setting up a build agent if you are either not using Visual Studio Online or want to use your own Mac with [cross-platform agent](#xplat) to build for iOS.
...
- **Visual Studio Online**: For Visual Studio Online, this is easy to do. Simply install the [Tools for Apache Cordova Extension](http://go.microsoft.com/fwlink/?LinkID=691188). * **TODO: Quick summary of installing.** *

- **TFS 2015 Update 1 and Earlier**: TFS 2015 Update 1 does not currently support installing VSO Extensions. Fortunately installation of the Cordova task is still relatively  easy to do by following these simple steps.
...
We'll assume for the purposes of this tutorial that we want to build our Cordova app for Android, iOS, and Windows and you want to build everything on Windows except iOS. We will accomplish this by using the concept of a "demand" in two separate build definitions to route the work to the correct OS.
...
6.  As an optional step, you can configure your build to upload the resulting build artifacts to your TFS or VSO instance for easy access.
...
1. First, just update the **Gulp Task(s)** option in the Gulp step above to reference your "test" task. A value of "scripts test" will first compile TypeScript (or anything else you have configured) and then run tests.
...
2.  Manually install only the pre-requisites needed for the specific platforms you intend to build. For example, you do not need to install Visual Studio at all if you only intend to target Android. See "Installing Dependencies" in the [general CI guide](http://go.microsoft.com/fwlink/?LinkID=691196) for details.
...
>**Troubleshooting Tip:** See ["Internet Access & Proxy Setup" in the general CI guide](http://go.microsoft.com/fwlink/?LinkID=5691197) if your build servers have limited Internet connectivity or require routing traffic through a proxy.
...
## More Information
* [Learn more about Tools for Apache Cordova](http://aka.ms/cordova)
