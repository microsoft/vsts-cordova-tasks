...
function configure(obj) {
    ...
    if (obj.loadCordovaModule !== undefined) loadCordovaModule = obj.loadCordovaModule;
...
function setupCordova(obj) {
    ...
    var versionPath = path.join(cordovaCache, cordovaVersion);
    if (!fs.existsSync(path.join(versionPath, "node_modules", CORDOVA))) {  // Fix: Check module is there not just root path
        ...
        var cmd = "npm install " + CORDOVA + "@" + cordovaVersion
...
function addPlatformsToProject(cordova, cordovaPlatforms) {
    var promise = Q();
    cordovaPlatforms.forEach(function (platform) {
        console.log(path.join(projectPath, "platforms", platform));
...
function getCurrentCordovaModulePath() {
    return cordovaModulePath;
}

function getCurrentCordovaVersion() {
...
module.exports = {
    ...
    getCurrentCordovaModulePath: getCurrentCordovaModulePath
};
