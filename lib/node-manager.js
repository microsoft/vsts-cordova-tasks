...
function setupNode(targetVersion, /*optional*/ installNpmOnWindows) {
    ...
    if (process.platform == "win32") {
        ...
        if (!fs.existsSync(nodePath)) {
            ...
            var args = ["-o", path.join(nodePath, "node.exe"), "https://nodejs.org/dist/v" + targetVersion + "/win-x86/node.exe"];
            promise = promise.then(function () {
                var spawnResult = spawn(curlPath, args, { stdio: "inherit" });
        ...
        if (installNpmOnWindows && !fs.existsSync(path.join(nodePath, "node_modules", "npm"))) {
            ...
            var args = ["/c cd " + nodePath.substr(0, 2) + " && cd \"" + nodePath + "\" && npm install --force npm@" + npmVersion];
            promise = promise.then(function () {
                var spawnResult = spawn(shellCmd, args, { stdio: "inherit" });
                if (spawnResult.status > 0) {
                    console.error("failed command: " + shellCmd);
                    process.exit(spawnResult.status);;
                }
            });
        }
