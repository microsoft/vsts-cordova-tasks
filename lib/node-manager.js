    ...
    if (process.platform == "win32") {
        ...
        if (!fs.existsSync(nodePath)) {
            ...
            if (semver.lt(targetVersion, "4.0.0")) {
                nodeDistPath = "https://nodejs.org/dist/v" + targetVersion + "/node.exe"
            } else {
                nodeDistPath = "https://nodejs.org/dist/v" + targetVersion + "/win-x86/node.exe";
            }

            shelljs.mkdir("-p", nodePath);
            var curlCmdArgs = ["-o", path.join(nodePath, "node.exe"), nodeDistPath];
